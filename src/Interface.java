...
public class Interface {
	...
	private void initialize() {
		...
		pnlPage.add(txtLocation, gbc_txtLocation);
		...
		JCheckBox checkBoxRoute = new JCheckBox("");//CechBoxRoute and checkBoxName are here to use it on checkBoxDoc
		
		JCheckBox checkBoxName = new JCheckBox("");
		
		JCheckBox checkBoxDoc = new JCheckBox("");
		checkBoxDoc.addMouseListener(new MouseAdapter() {

			@Override
			public void mouseClicked(MouseEvent e) {
				if (checkBoxDoc.isSelected() == false) {
					txtRoute.setEditable(false);
					txtName.setEditable(false);
				}
				if (checkBoxName.isSelected() && checkBoxDoc.isSelected()) {
					txtName.setEditable(true);
				} else {
					txtName.setEditable(false);
					txtName.setText("Password");
				}
				if (checkBoxRoute.isSelected() && checkBoxDoc.isSelected()) {
		...
		checkBoxRoute.addMouseListener(new MouseAdapter() {
			@Override
		...
		checkBoxName.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				if (checkBoxName.isSelected() && checkBoxDoc.isSelected()) {
					txtName.setEditable(true);
		...
		btnGeneratePassword.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				// This happens when you click on the "generate password"
				// button.
				// String lng is just to get the text from the spinner.
				String lng = spinnerLength.getValue().toString();
				// Integer length is the text got with lng parsed into a
				// integer.
				...
				String pass= finalPassword;
				String location=txtLocation.getText();
				String route=txtRoute.getText();
				String name=txtName.getText();
				if(checkBoxDoc.isSelected()){
				try {
					Principal.generateTxt(pass, location, route, name);
